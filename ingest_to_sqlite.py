...
Loads synthetic e-commerce CSV files from `data/` into a
normalized SQLite database located at `database/ecommerce.db`.

Usage:
    python ingest_to_sqlite.py
    python ingest_to_sqlite.py --ltv
    python ingest_to_sqlite.py --db-path database/ecommerce.db --replace
"""
...
def read_csv_rows(path: Path | str) -> Iterable[dict]:
...
def insert_rows(
    ...
    cur = conn.cursor()
    cur.executemany(
        sql, ([row.get(c, None) for c in columns] for row in rows)
    )
...
def ingest(
    ...
    try:
        for table, path in paths.items():
            ...
            if not rows:
                print(f"Warning: No rows found in {path}; skipping.")
                continue
            ...
            with conn:
                # Filter columns to only those that exist in the CSV
                available_cols = [c for c in col_map[table] if c in rows[0]]
                insert_rows(conn, table, rows, available_cols)
        ...
        if write_ltv:
            ltv_path = data_dir / "customer_ltv.csv"
            cur = conn.cursor()
            cur.execute(
                """
                SELECT
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Ingest synthetic e-commerce CSVs into SQLite database."
    )
    parser.add_argument(
        "--data-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Path to SQLite database (default: database/ecommerce.db)",
    )
    parser.add_argument(
        "--replace",
        action="store_true",
        help="Remove existing DB before ingestion",
    )
    parser.add_argument(
        "--ltv",
        dest="write_ltv",
        action="store_true",
        help="Generate customer_ltv.csv report",
    )
