...
Creates five CSV files in the `data/` folder:
- customers.csv
- products.csv
- orders.csv
- order_items.csv
...
Usage Example:
    python generate_data.py --seed 42 --scale 1
...
def generate_products(fake: Faker, n_products: int) -> pd.DataFrame:
    ...
    for pid in range(1, n_products + 1):
        category = np.random.choice(
            categories, p=[0.18, 0.14, 0.12, 0.12, 0.18, 0.14, 0.12]
        )
...
def generate_reviews(
    ...
    for rid in range(1, n_reviews + 1):
        ...
        rating = int(
            np.random.choice([1, 2, 3, 4, 5], p=[0.05, 0.05, 0.15, 0.4, 0.35])
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Scale factor; multiplies base sizes",
    )
    ...
    orders_df, items_df = generate_orders_and_items(
        fake, n_orders, customers_df, products_df
    )
    reviews_df = generate_reviews(fake, customers_df, products_df, n_reviews)

    # Save as CSV
    customers_df.to_csv(outdir / "customers.csv", index=False)
    products_df.to_csv(outdir / "products.csv", index=False)
    orders_df.to_csv(outdir / "orders.csv", index=False)
    items_df.to_csv(outdir / "order_items.csv", index=False)
    reviews_df.to_csv(outdir / "reviews.csv", index=False)

    print("\nSynthetic Ecom Data Generated Successfully!")
    print(f"Output Directory: {outdir}")
    print(
        f"Customers: {len(customers_df)}, items: {len(items_df)}, "
        f"orders: {len(orders_df)}, products: {len(products_df)}, "
